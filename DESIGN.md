...
- Primary paths: learn about the maahad, explore education and facilities, read activities, view the gallery, consult FAQ, and contact the institution.
...
- Document opening/refresh: a full-screen navy-blue splash presents the official logo in a white medallion with a thin rotating gold ring. It stays for 2.2 seconds (up to 2.8 seconds for slow hero media), then fades/lifts away over 650ms. Only after the exit is the main page unlocked and its entrance played. In-page links never replay the splash. The old session-storage skip has been removed so opening or refreshing the website reliably shows the requested introduction. Reduced motion skips it, including when enabled while loading; a 4.2-second watchdog releases content if the main script fails. No browser storage is required for the intro.
...
- All active people imagery depicts an all-female maahad environment. Every visible santriwati and ustadzah, including background figures, wears a navy niqab/cadar. No male-student assets are referenced by the page.
...
- The inherited stock MP4 did not load reliably and was removed. No official video is present in the project. Set `data-video-src` on `#profileVideo` in `an-najah-3.html` to the official MP4 path or URL; the player loads only after a visitor opens it. Until configured, a designed availability message and program link are shown.
...
- Program names, achievements, contact details, admissions information, and every institutional claim must be verified by Maahad An-Najah before production launch.
...
- Desktop 1269 x 720: two-line hero headline, primary CTAs above the fold, no horizontal overflow.
- Mobile 390 x 844: readable hero, functioning menu and theme switch, no horizontal overflow; all four gallery rows keep an 18px gap, including the taller portrait image.
- Small mobile 320 x 740: removed a minimum-width constraint and adjusted panel typography; no horizontal page overflow after retesting.
...
- Revision tests: run the single-process suite with `node tests/profile.test.mjs`; all 15 tests pass. The isolated `node --test` runner cannot spawn subprocesses in this sandbox, so the suite is executed directly with the same Node test API.
- The tests cover light default, persisted/invalid theme values, document-load splash, reduced-motion splash skipping, blocked storage, fail-open timeout, required sections, removal of admissions, local assets, anchors, and dummy-content disclosures.
...
- Splash refinement: browser checks verify the full-screen logo layer, animated ring, inert main content during loading, and automatic reveal/unlock. Fake-clock tests cover the 2.2-second hold plus 650ms exit, failed decoding, stalled media, reduced motion at startup, and reduced motion enabled mid-intro.
- Final verification: direct single-process unit suite 15/15, JavaScript syntax check successful, strict static audit 0 errors/0 warnings. Lighthouse and OS-level reduced-transparency/high-contrast emulation were not run; no performance-score claim is made.
